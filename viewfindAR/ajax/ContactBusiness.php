...
$result_user_mail = mysql_query($sql_user_mail);
...
$result_user_messg = mysql_query($sql_user_messg);
...
//        echo '$business_mail=',$business_mail,'$tag_name=',$tag_name;
        ...
        if ($to_business == 'true') {
            ...
            $sql_history = "INSERT INTO `app_history`(`app_history`.`HISTORY_ID`, `app_history`.`ACTION_TYPE`, `app_history`.`USER_ID`, `app_history`.`TAG_ID`, `app_history`.`RESPONSE`, `app_history`.`COMMENTS`, `app_history`.`CREATED_DATE`)
                            VALUES (NULL, 'Contact Business', '" . $user_id . "', '" . $tag_id . "', 'Success', 'Mail has been Sent to the business succesfully', NOW())";
...
//        echo '$sql_history=', $sql_history;
            ...
            ######################################### Update History Table #####################################################################
            $sql_history = "INSERT INTO `app_history`(`app_history`.`HISTORY_ID`, `app_history`.`ACTION_TYPE`, `app_history`.`USER_ID`, `app_history`.`TAG_ID`, `app_history`.`RESPONSE`, `app_history`.`COMMENTS`, `app_history`.`CREATED_DATE`)
                            VALUES (NULL, 'Contact Business', '" . $user_id . "', '" . $tag_id . "', 'Fail', 'Problem While Contacting Business', NOW())";
...
//        echo '$sql_history=', $sql_history;
        ...
        ######################################### Update History Table #####################################################################
        $sql_history = "INSERT INTO `app_history`(`app_history`.`HISTORY_ID`, `app_history`.`ACTION_TYPE`, `app_history`.`USER_ID`, `app_history`.`TAG_ID`, `app_history`.`RESPONSE`, `app_history`.`COMMENTS`, `app_history`.`CREATED_DATE`)
                            VALUES (NULL, 'Contact Business', '" . $user_id . "', '" . $tag_id . "', 'Fail', 'User Mail Or Business mail is not present with the application', NOW())";
...
//        echo '$sql_history=', $sql_history;
        ...
        if ($mob) {
            ...
            echo 'Cannot contact business Please Contact Admin1';
        }
    }
...
//    sendSMS_user($user_phone_no);
    ...
    ######################################### Update History Table #####################################################################
    $sql_history = "INSERT INTO `app_history`(`app_history`.`HISTORY_ID`, `app_history`.`ACTION_TYPE`, `app_history`.`USER_ID`, `app_history`.`TAG_ID`, `app_history`.`RESPONSE`, `app_history`.`COMMENTS`, `app_history`.`CREATED_DATE`)
                            VALUES (NULL, 'Contact Business', '" . $user_id . "', '" . $tag_id . "', 'Fail', 'Multiple Business or No business associated with the tag', NOW())";
...
//        echo '$sql_history=', $sql_history;
    ...
    if ($mob) {
        ...
        echo 'Cannot contact business Please Contact Admin';
    }
}
?>